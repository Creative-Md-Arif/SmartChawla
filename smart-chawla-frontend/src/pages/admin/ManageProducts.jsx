...
import { 
  ...
  Package, Filter, RefreshCw, Star, Download,
  ToggleLeft, ToggleRight, AlertCircle, CheckCircle,
  ChevronDown, ChevronUp, BarChart3, Calendar, User, Tag, Weight,
  Image as ImageIcon
} from 'lucide-react';
...
const ManageProducts = () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        setStats({
          ...
          outOfStock: response.data.products?.filter(p => p.stock === 0).length || 0,
          featured: response.data.products?.filter(p => p.isFeatured).length || 0
        });
      }
    } catch (err) {
  ...
  }, [currentPage, limit, searchQuery, selectedCategory, stockFilter, featuredFilter, priceRange, sortBy]);
  ...
  const fetchSuggestions = async (query) => {
    ...
    try {
      const response = await axiosInstance.get(`/products/search-suggestions?q=${query}&limit=5`);
  ...
  const getCreateSubCategories = () => {
    const selectedCat = categories.find(cat => cat._id === createFormData.category);
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      if (createFormData.discountPrice && parseFloat(createFormData.discountPrice) > 0) {
      ...
      if (createFormData.subCategory) data.append('subCategory', createFormData.subCategory);
      if (createFormData.sku) data.append('sku', createFormData.sku.trim());
      if (createFormData.metaTitle) data.append('metaTitle', createFormData.metaTitle.trim());
      if (createFormData.metaDescription) data.append('metaDescription', createFormData.metaDescription.trim());
      ...
      if (createFormData.specifications.weight) {
        data.append('specifications[weight]', createFormData.specifications.weight);
        data.append('specifications[weightUnit]', createFormData.specifications.weightUnit);
      }
      if (createFormData.specifications.material) {
        data.append('specifications[material]', createFormData.specifications.material.trim());
      }
      if (createFormData.specifications.color) {
        data.append('specifications[color]', createFormData.specifications.color.trim());
      }
      if (createFormData.specifications.warranty) {
        data.append('specifications[warranty]', createFormData.specifications.warranty.trim());
      }
      if (createFormData.specifications.dimensions.length) {
        data.append('specifications[dimensions][length]', createFormData.specifications.dimensions.length);
      }
      if (createFormData.specifications.dimensions.width) {
        data.append('specifications[dimensions][width]', createFormData.specifications.dimensions.width);
      }
      if (createFormData.specifications.dimensions.height) {
        data.append('specifications[dimensions][height]', createFormData.specifications.dimensions.height);
      }
      ...
      showNotification(err.response?.data?.message || 'প্রোডাক্ট তৈরি ব্যর্থ হয়েছে', 'error');
    } finally {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axiosInstance.delete(`/products/${deleteModal.product._id}?permanent=${deleteModal.permanent}`);
      showNotification(
  ...
  const handleStockUpdate = async (productId, newStock) => {
    try {
      await axiosInstance.patch(`/products/${productId}/stock`, { stock: Number(newStock) });
  ...
  const toggleFeatured = async (product) => {
    try {
      ...
      showNotification(product.isFeatured ? 'ফিচার্ড থেকে সরানো হয়েছে' : 'ফিচার্ড করা হয়েছে');
      fetchProducts();
    } catch (err) {
  ...
  const toggleActive = async (product) => {
    try {
      ...
      showNotification(product.isActive ? 'ডিঅ্যাক্টিভেট করা হয়েছে' : 'অ্যাক্টিভেট করা হয়েছে');
      fetchProducts();
    } catch (err) {
      showNotification('আপডেট করতে সমস্যা হয়েছে', 'error');
    }
  ...
  const handleBulkDelete = async (permanent = false) => {
    ...
    try {
      ...
      showNotification(`${selectedProducts.length}টি প্রোডাক্ট মুছে ফেলা হয়েছে`);
      setSelectedProducts([]);
      fetchProducts();
    } catch (err) {
  ...
  const formatDate = (date) => new Date(date).toLocaleDateString('bn-BD', {
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-800 mb-2">প্রোডাক্ট ম্যানেজমেন্ট</h1>
          <p className="text-gray-600">সমস্ত প্রোডাক্ট দেখুন, সম্পাদনা করুন এবং পরিচালনা করুন</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-600">মোট প্রোডাক্ট</p>
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between mb-4">
          ...
          <div className="relative flex-1 min-w-[300px]" ref={searchRef}>
            ...
            {showSuggestions && suggestions.length > 0 && (
              <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg mt-1 shadow-lg max-h-60 overflow-auto">
                {suggestions.map((suggestion, idx) => (
                  ...
                  >
                    {suggestion.images?.[0]?.url && (
                      <img src={suggestion.images[0].url} alt="" className="w-8 h-8 object-cover rounded" />
                    )}
                    <div>
      ...
      {selectedProducts.length > 0 && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-4 flex items-center justify-between">
          <span className="text-blue-800 font-medium">{selectedProducts.length}টি প্রোডাক্ট নির্বাচিত</span>
          <div className="flex gap-2">
            <button
          ...
          <>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  {products.map((product) => (
                    <tr key={product._id} className={`hover:bg-gray-50 ${!product.isActive ? 'bg-gray-100' : ''}`}>
                      ...
                      <td className="px-6 py-4">
                        ...
                        <div className="text-xs text-gray-400 mt-1">{formatDate(product.createdAt)}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                          {product.category?.name?.bn || product.category?.name?.en || 'N/A'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <input
                          ...
                          className={`w-20 px-2 py-1 border rounded text-center ${
                            product.stock === 0 ? 'border-red-300 bg-red-50' : 'border-gray-300'
                          }`}
                          min="0"
                        />
                      </td>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            ...
                            className={`p-1 rounded ${product.isFeatured ? 'text-yellow-500' : 'text-gray-400 hover:text-yellow-500'}`}
                          >
                            <Star className={`w-5 h-5 ${product.isFeatured ? 'fill-current' : ''}`} />
                          </button>
                          <button onClick={() => viewProduct(product.slug)} className="p-1 text-blue-600 hover:text-blue-900">
                            <Eye className="w-5 h-5" />
                          </button>
                          <button onClick={() => openUpdateModal(product)} className="p-1 text-indigo-600 hover:text-indigo-900">
            ...
            <div className="bg-gray-50 px-6 py-4 flex items-center justify-between border-t border-gray-200">
              <div className="text-sm text-gray-700">
                মোট <span className="font-medium">{totalProducts}</span>টি প্রোডাক্ট
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                <span className="px-4 py-2">
                  পেজ <span className="font-medium">{currentPage}</span> / {totalPages}
                </span>
                <button
      ...
      {isCreateModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto">
          <div className="min-h-screen px-4 text-center">
            <div className="fixed inset-0 transition-opacity" onClick={closeCreateModal} />
            ...
            <div className="inline-block w-full max-w-5xl my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              ...
              <div className="flex justify-between items-center p-6 border-b">
                <h2 className="text-2xl font-bold text-gray-900">নতুন প্রোডাক্ট তৈরি</h2>
              ...
              <form onSubmit={handleCreateSubmit} className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-4">
                    <h3 className="font-semibold text-gray-700 border-b pb-2">মূল তথ্য</h3>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setCreateFormData({...createFormData, category: e.target.value, subCategory: ''})}
                        className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      >
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">মূল্য (Regular Price) *</label>
                        <div className="relative">
                          ...
                          <input
                            ...
                            onChange={(e) => handleCreatePriceChange('price', e.target.value)}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">ডিসকাউন্ট মূল্য (Sale Price)</label>
                        <div className="relative">
                          <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500">৳</span>
                          <input
                            type="text"
                            ...
                            onChange={(e) => handleCreatePriceChange('discountPrice', e.target.value)}
                            className={`w-full pl-8 pr-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 ${createPriceError ? 'border-red-500' : 'border-gray-300'}`}
                            placeholder="0.00"
                          />
                        </div>
                    ...
                    {createFormData.price && !createPriceError && (
                      <div className="bg-blue-50 border border-blue-200 p-3 rounded-lg">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="font-semibold">৳{parseFloat(createFormData.price).toLocaleString('bn-BD')}</span>
                        </div>
                        {createFormData.discountPrice && parseFloat(createFormData.discountPrice) > 0 && (
                          <>
                            <div className="flex justify-between items-center mt-1">
                              <span className="text-sm text-gray-600">ডিসকাউন্ট:</span>
                              <span className="font-semibold text-green-600">৳{parseFloat(createFormData.discountPrice).toLocaleString('bn-BD')}</span>
                            </div>
                            <div className="flex justify-between items-center mt-1 pt-1 border-t border-blue-200">
                              <span className="text-sm font-medium">সেভ করবেন:</span>
                              <span className="font-bold text-blue-600">৳{(parseFloat(createFormData.price) - parseFloat(createFormData.discountPrice)).toLocaleString('bn-BD')}</span>
                            </div>
                          </>
                        )}
                      </div>
                  ...
                  {/* Additional Info */}
                  <div className="space-y-4">
                    <h3 className="font-semibold text-gray-700 border-b pb-2">অতিরিক্ত তথ্য</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">ওজন</label>
                        <div className="flex gap-2">
                          <input
                            type="number"
                            step="0.01"
                            ...
                            onChange={(e) => setCreateFormData({
                              ...createFormData,
                              specifications: {...createFormData.specifications, weight: e.target.value}
                            })}
                          ...
                          <select
                            ...
                            onChange={(e) => setCreateFormData({
                              ...createFormData,
                              specifications: {...createFormData.specifications, weightUnit: e.target.value}
                            })}
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">ম্যাটেরিয়াল</label>
                        <input
                          type="text"
                          value={createFormData.specifications.material}
                          onChange={(e) => setCreateFormData({
                            ...createFormData,
                            specifications: {...createFormData.specifications, material: e.target.value}
                          })}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        />
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setCreateFormData({
                            ...createFormData,
                            specifications: {...createFormData.specifications, color: e.target.value}
                          })}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                        />
                      </div>
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">ওয়ারেন্টি</label>
                        <input
                          type="text"
                          value={createFormData.specifications.warranty}
                          onChange={(e) => setCreateFormData({
                            ...createFormData,
                            specifications: {...createFormData.specifications, warranty: e.target.value}
                          })}
                          className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">ডাইমেনশন (L×W×H)</label>
                      <div className="grid grid-cols-3 gap-2">
                        <input
                          type="number"
                          placeholder="দৈর্ঘ্য"
                          value={createFormData.specifications.dimensions.length}
                          onChange={(e) => setCreateFormData({
                            ...createFormData,
                            specifications: {
                              ...createFormData.specifications,
                              dimensions: {...createFormData.specifications.dimensions, length: e.target.value}
                            }
                          })}
                        ...
                        <input
                          ...
                          onChange={(e) => setCreateFormData({
                            ...
                            specifications: {
                              ...createFormData.specifications,
                              dimensions: {...createFormData.specifications.dimensions, width: e.target.value}
                            }
                          })}
                          className="px-3 py-2 border border-gray-300 rounded-lg"
                        />
                        <input
                          type="number"
                          placeholder="উচ্চতা"
                          value={createFormData.specifications.dimensions.height}
                          onChange={(e) => setCreateFormData({
                            ...createFormData,
                            specifications: {
                              ...createFormData.specifications,
                              dimensions: {...createFormData.specifications.dimensions, height: e.target.value}
                            }
                          })}
                          className="px-3 py-2 border border-gray-300 rounded-lg"
                        />
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1">ট্যাগ (কমা দিয়ে আলাদা করুন)</label>
                      <input
                        type="text"
                ...
                <div className="mt-6">
                  ...
                  <div className="flex items-center justify-center w-full">
                    <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 ${createImagePreview.length >= 5 ? 'border-gray-200 opacity-50 cursor-not-allowed' : 'border-gray-300'}`}>
                      <div className="flex flex-col items-center justify-center pt-5 pb-6">
                        <ImageIcon className="w-8 h-8 mb-3 text-gray-400" />
                        <p className="text-sm text-gray-500">ছবি আপলোড করতে ক্লিক করুন</p>
                        <p className="text-xs text-gray-400">(সর্বোচ্চ ৫টি, প্রতিটি ৫MB এর কম)</p>
                      </div>
                      <input
                        type="file"
                        multiple
                    ...
                    <div className="flex gap-4 mt-4 flex-wrap">
                      {createImagePreview.map((src, idx) => (
                        <div key={idx} className="relative">
                          <img src={src} alt={`Preview ${idx}`} className="h-20 w-20 object-cover rounded-lg" />
                          <button
                            type="button"
                ...
                <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">মেটা টাইটেল</label>
                    <input
                      type="text"
                      value={createFormData.metaTitle}
                      onChange={(e) => setCreateFormData({...createFormData, metaTitle: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      maxLength={70}
                      placeholder="SEO টাইটেল (সর্বোচ্চ ৭০ অক্ষর)"
                    />
                    <span className="text-xs text-gray-500">{createFormData.metaTitle.length}/70</span>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">মেটা ডেসক্রিপশন</label>
                    <textarea
                      value={createFormData.metaDescription}
                      onChange={(e) => setCreateFormData({...createFormData, metaDescription: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                      rows={2}
                      maxLength={160}
                      placeholder="SEO ডেসক্রিপশন (সর্বোচ্চ ১৬০ অক্ষর)"
                    />
                    <span className="text-xs text-gray-500">{createFormData.metaDescription.length}/160</span>
                  </div>
                </div>
                ...
                {createUploadProgress > 0 && createUploadProgress < 100 && (
                  <div className="mt-4">
                    <div className="w-full bg-gray-200 rounded-full h-2.5">
                      <div className="bg-blue-600 h-2.5 rounded-full transition-all duration-300" style={{ width: `${createUploadProgress}%` }}></div>
                    </div>
                    <p className="text-sm text-gray-600 mt-1 text-center">{createUploadProgress}% আপলোড হয়েছে</p>
                  </div>
                )}
                ...
                <div className="mt-8 flex justify-end gap-4">
                  ...
                  >
                    {createSubmitting && <RefreshCw className="w-4 h-4 animate-spin" />}
                    তৈরি করুন
                  </button>
                </div>
              </form>
      ...
      {viewingProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 overflow-y-auto">
          <div className="min-h-screen px-4 text-center">
            <div className="fixed inset-0 transition-opacity" onClick={() => setViewingProduct(null)} />
            <div className="inline-block w-full max-w-4xl my-8 text-left align-middle transition-all transform bg-white shadow-xl rounded-lg">
              <div className="flex justify-between items-start p-6 border-b">
                <div>
                  ...
                  <p className="text-gray-500 mt-1">Slug: {viewingProduct.slug}</p>
                </div>
                <div className="flex gap-2">
                  <button
                    onClick={() => {
      ...
      {deleteModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-bold text-gray-900 mb-4">নিশ্চিত করুন</h3>
            <p className="text-gray-600 mb-4">
              আপনি কি নিশ্চিতভাবে <strong>{deleteModal.product?.name}</strong> মুছে ফেলতে চান?
            </p>
            <div className="flex gap-3 justify-end">
              ...
              >
                {deleteModal.permanent ? 'স্থায়ীভাবে মুছুন' : 'ডিঅ্যাক্টিভেট করুন'}
              </button>
            </div>
          </div>
        </div>
